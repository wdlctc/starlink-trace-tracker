...
def main():
    ...
    measure_trace = diff(snr_data_array[0], snr_data_array[1])

    if args.debug:
        print(measure_trace)

    # find starlink satellite trace which is closest to the difference
    times = ts.linspace(timeline[0], timeline[1], num=15)
    min_distance = 100000
    min_index = 0
    all_trace = []
    all_sat = []
    ...
    plt.savefig('starlink_match_plots.png')

    print("from " + str(timeline[0].utc_datetime()) + " to " + str(timeline[1].utc_datetime())) 
