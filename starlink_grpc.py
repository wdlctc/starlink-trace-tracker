...
def restart_control(context: Optional[ChannelContext] = None):
    """ Request dish restart control operation
    ...
    def grpc_call(channel: grpc.Channel) -> None:
        if imports_pending:
            resolve_imports(channel)
        stub = device_pb2_grpc.DeviceStub(channel)
        stub.Handle(device_pb2.Request(get_ping={}), timeout=REQUEST_TIMEOUT)
